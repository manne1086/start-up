    ...
    <section className="flex flex-col items-center text-center mt-20 px-4 w-full max-w-4xl mx-auto flex-1">
      ...
      <div className="w-full max-w-3xl relative group">
        <textarea 
          ...
          className="w-full bg-[#12121A] border-2 border-white/20 text-white placeholder-gray-500 px-8 py-6 rounded-full focus:outline-none focus:border-[#6C47FF] focus:shadow-[6px_6px_0px_#6C47FF] transition-all resize-none text-lg font-medium shadow-[4px_4px_0px_transparent]"
        ></textarea>
        
        <div className="flex flex-wrap justify-center gap-4 mt-8">
          {['HealthTech SaaS', 'D2C Fintech', 'EdTech B2B'].map((tag) => (
            <button key={tag} className="border-2 border-white/20 px-6 py-2 rounded-full text-sm font-bold text-gray-400 hover:border-[#00D4AA] hover:text-[#00D4AA] hover:bg-[#00D4AA]/10 transition-colors shadow-[2px_2px_0px_transparent] hover:shadow-[2px_2px_0px_#00D4AA]">
              {tag}
            </button>
          ))}
        </div>
